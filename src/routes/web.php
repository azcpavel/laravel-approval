...
Route::group(['prefix' => config('approval-config.route-prefix'),'namespace' => 'Exceptio\ApprovalPermission\Http\Controllers', 
    ...
    Route::post('approval-request/comment/{approvalRequest}','ApprovalRequestController@commentLevel')->name(config('approval-config.route-name-request-prefix').'.comment_level');
});
